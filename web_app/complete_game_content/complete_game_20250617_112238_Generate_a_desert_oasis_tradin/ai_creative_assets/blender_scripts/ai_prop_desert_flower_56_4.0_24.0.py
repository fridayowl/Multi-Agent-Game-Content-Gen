...
# Prop #57: desert_flower
# AI Description: A unique desert desert_flower with distinctive characteristics
# Style Parameters: {'shape': 'round', 'size': 'large', 'texture': 'rough', 'color_scheme': 'monochrome', 'complexity': 'intricate'}
...
# AI Texture paths
ai_textures = {"surface": "complete_game_content/complete_game_20250617_112238_Generate_a_desert_oasis_tradin/ai_creative_assets/ai_textures/generic_desert_e2fb947d.png"}

# AI-Generated Geometry Parameters
HEIGHT_MULT = 1.4003844173090738
WIDTH_MULT = 0.8047535441541078
COMPLEXITY = "complex"
DETAIL_COUNT = 4
ASYMMETRY = 0.2994561905056916
...
def create_ai_unique_desert_flower():
    ...
    if len(prop_objects) > 1:
        ...
        if prop_objects:
            ...
            final_prop.name = f"AI_desert_flower_56"
...
def create_ai_tree():
    ...
    if trunk_style == "twisted":
        ...
        for i in range(max(2, DETAIL_COUNT // 2)):
            ...
            bpy.ops.mesh.primitive_cylinder_add(
                ...
                location=(4.0, 24.0, i * segment_height + segment_height/2)
            )
        ...
        bpy.ops.mesh.primitive_cylinder_add(
            ...
            location=(4.0, 24.0, trunk_height * 0.4)
        )
        ...
        bpy.ops.mesh.primitive_cylinder_add(
            ...
            location=(4.0, 24.0, trunk_height * 0.3)
        )
        trunk = bpy.context.active_object
        ...
        for i in range(2):
            ...
            bpy.ops.mesh.primitive_cylinder_add(
                ...
                location=(4.0 + offset_x, 24.0 + offset_y, trunk_height * 0.75)
            )
        ...
        bpy.ops.mesh.primitive_cylinder_add(
            ...
            radius=trunk_radius,
            depth=trunk_height,
            location=(4.0, 24.0, trunk_height/2)
        )
        trunk = bpy.context.active_object
    ...
    if canopy_shape == "round":
        bpy.ops.mesh.primitive_ico_sphere_add(
            subdivisions=2,
            radius=canopy_size,
            location=(4.0, 24.0, canopy_height)
        )
        ...
        for i in range(DETAIL_COUNT):
            ...
            bpy.ops.mesh.primitive_ico_sphere_add(
                subdivisions=1,
                radius=size,
                location=(4.0 + offset_x, 24.0 + offset_y, canopy_height + offset_z)
            )
...
def create_ai_rock():
    ...
    if rock_shape == "angular":
        bpy.ops.mesh.primitive_cube_add(
            size=base_size,
            location=(4.0, 24.0, rock_height/2)
        )
        ...
        bpy.ops.mesh.primitive_cone_add(
            ...
            location=(4.0, 24.0, rock_height * 0.75)
        )
        rock = bpy.context.active_object
        ...
        for i in range(DETAIL_COUNT // 2):
            ...
            bpy.ops.mesh.primitive_cone_add(
                ...
                location=(4.0 + offset_x, 24.0 + offset_y, rock_height * 0.35)
            )
    ...
    if bush_shape == "spreading":
        ...
        for i in range(DETAIL_COUNT):
            ...
            bpy.ops.mesh.primitive_ico_sphere_add(
                subdivisions=1,
                radius=sphere_size,
                location=(4.0 + offset_x, 24.0 + offset_y, bush_height * 0.4)
            )
            sphere = bpy.context.active_object
...
def create_ai_well():
    ...
    bpy.ops.mesh.primitive_cylinder_add(
        ...
        location=(4.0, 24.0, 0.5 * HEIGHT_MULT)
    )
    ...
    if random.random() > 0.5:  # 50% chance of roof
        bpy.ops.mesh.primitive_cone_add(
            ...
            radius2=0.1,
            depth=0.8,
            location=(4.0, 24.0, HEIGHT_MULT + 0.4)
        )
...
try:
    ...
    print(f"🎨 Style: {'shape': 'round', 'size': 'large', 'texture': 'rough', 'color_scheme': 'monochrome', 'complexity': 'intricate'}")
