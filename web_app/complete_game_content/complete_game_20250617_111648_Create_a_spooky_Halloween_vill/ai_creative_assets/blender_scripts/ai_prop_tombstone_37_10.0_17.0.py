...
# Prop #38: tombstone
# AI Description: A unique spooky tombstone with distinctive characteristics
...
# AI Texture paths
ai_textures = {"surface": "complete_game_content/complete_game_20250617_111648_Create_a_spooky_Halloween_vill/ai_creative_assets/ai_textures/generic_spooky_21a97dfd.png"}

# AI-Generated Geometry Parameters
HEIGHT_MULT = 1.0967511364386215
WIDTH_MULT = 1.1356888133322156
COMPLEXITY = "medium"
DETAIL_COUNT = 5
ASYMMETRY = 0.3307342026218629

print(f"🎨 Creating AI-Designed tombstone:")
print(f"   📐 Height: {HEIGHT_MULT:.2f}x, Width: {WIDTH_MULT:.2f}x")
print(f"   🔧 Complexity: {COMPLEXITY}")
print(f"   ✨ Details: {DETAIL_COUNT}")
print(f"   🎭 Asymmetry: {ASYMMETRY:.2f}")

def create_ai_unique_tombstone():
    """Create tombstone with AI-determined unique characteristics"""
    
    prop_objects = []
    
    if "tombstone" in ["tree", "oak_tree", "dead_tree", "palm_tree"]:
        # Create AI-designed tree
        prop_objects = create_ai_tree()
        
    elif "tombstone" in ["rock", "stone", "boulder"]:
        # Create AI-designed rock
        prop_objects = create_ai_rock()
        
    elif "tombstone" in ["bush", "shrub", "plant"]:
    ...
    if len(prop_objects) > 1:
        ...
        if prop_objects:
            ...
            final_prop.name = f"AI_tombstone_37"
            
            print(f"✅ Created AI tombstone with {len(prop_objects)} components")
...
def create_ai_tree():
    ...
    if trunk_style == "twisted":
        ...
        for i in range(max(2, DETAIL_COUNT // 2)):
            ...
            bpy.ops.mesh.primitive_cylinder_add(
                ...
                location=(10.0, 17.0, i * segment_height + segment_height/2)
            )
        ...
        bpy.ops.mesh.primitive_cylinder_add(
            ...
            location=(10.0, 17.0, trunk_height * 0.4)
        )
        ...
        bpy.ops.mesh.primitive_cylinder_add(
            ...
            location=(10.0, 17.0, trunk_height * 0.3)
        )
        trunk = bpy.context.active_object
        ...
        for i in range(2):
            ...
            bpy.ops.mesh.primitive_cylinder_add(
                ...
                location=(10.0 + offset_x, 17.0 + offset_y, trunk_height * 0.75)
            )
        ...
        bpy.ops.mesh.primitive_cylinder_add(
            ...
            radius=trunk_radius,
            depth=trunk_height,
            location=(10.0, 17.0, trunk_height/2)
        )
        trunk = bpy.context.active_object
    ...
    if canopy_shape == "round":
        bpy.ops.mesh.primitive_ico_sphere_add(
            subdivisions=2,
            radius=canopy_size,
            location=(10.0, 17.0, canopy_height)
        )
        ...
        for i in range(DETAIL_COUNT):
            ...
            bpy.ops.mesh.primitive_ico_sphere_add(
                subdivisions=1,
                radius=size,
                location=(10.0 + offset_x, 17.0 + offset_y, canopy_height + offset_z)
            )
...
def create_ai_rock():
    ...
    if rock_shape == "angular":
        bpy.ops.mesh.primitive_cube_add(
            size=base_size,
            location=(10.0, 17.0, rock_height/2)
        )
        ...
        bpy.ops.mesh.primitive_cone_add(
            ...
            location=(10.0, 17.0, rock_height * 0.75)
        )
        rock = bpy.context.active_object
        ...
        for i in range(DETAIL_COUNT // 2):
            ...
            bpy.ops.mesh.primitive_cone_add(
                ...
                location=(10.0 + offset_x, 17.0 + offset_y, rock_height * 0.35)
            )
    ...
    if bush_shape == "spreading":
        ...
        for i in range(DETAIL_COUNT):
            ...
            bpy.ops.mesh.primitive_ico_sphere_add(
                subdivisions=1,
                radius=sphere_size,
                location=(10.0 + offset_x, 17.0 + offset_y, bush_height * 0.4)
            )
            sphere = bpy.context.active_object
        ...
        bpy.ops.mesh.primitive_ico_sphere_add(
            subdivisions=2,
            radius=bush_size,
            location=(10.0, 17.0, bush_height/2)
        )
...
def create_ai_well():
    ...
    bpy.ops.mesh.primitive_cylinder_add(
        ...
        location=(10.0, 17.0, 0.5 * HEIGHT_MULT)
    )
    ...
    if random.random() > 0.5:  # 50% chance of roof
        bpy.ops.mesh.primitive_cone_add(
            ...
            radius2=0.1,
            depth=0.8,
            location=(10.0, 17.0, HEIGHT_MULT + 0.4)
        )
...
try:
    created_objects = create_ai_unique_tombstone()
    print(f"✅ Successfully created AI tombstone with {len(created_objects)} components")
    ...
    print(f"❌ Error creating AI tombstone: {e}")
    import traceback
    traceback.print_exc()
