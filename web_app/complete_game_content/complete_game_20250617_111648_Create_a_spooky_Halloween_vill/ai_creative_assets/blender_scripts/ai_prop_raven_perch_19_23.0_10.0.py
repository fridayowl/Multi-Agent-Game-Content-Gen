...
# Prop #20: raven_perch
# AI Description: A unique spooky raven_perch with distinctive characteristics
...
# AI Texture paths
ai_textures = {"surface": "complete_game_content/complete_game_20250617_111648_Create_a_spooky_Halloween_vill/ai_creative_assets/ai_textures/generic_spooky_3ef0f0f6.png"}

# AI-Generated Geometry Parameters
HEIGHT_MULT = 1.265492155310525
WIDTH_MULT = 0.8354576540010825
COMPLEXITY = "simple"
DETAIL_COUNT = 2
ASYMMETRY = 0.3554182256784334

print(f"🎨 Creating AI-Designed raven_perch:")
print(f"   📐 Height: {HEIGHT_MULT:.2f}x, Width: {WIDTH_MULT:.2f}x")
print(f"   🔧 Complexity: {COMPLEXITY}")
print(f"   ✨ Details: {DETAIL_COUNT}")
print(f"   🎭 Asymmetry: {ASYMMETRY:.2f}")

def create_ai_unique_raven_perch():
    """Create raven_perch with AI-determined unique characteristics"""
    
    prop_objects = []
    
    if "raven_perch" in ["tree", "oak_tree", "dead_tree", "palm_tree"]:
        # Create AI-designed tree
        prop_objects = create_ai_tree()
        
    elif "raven_perch" in ["rock", "stone", "boulder"]:
        # Create AI-designed rock
        prop_objects = create_ai_rock()
        
    elif "raven_perch" in ["bush", "shrub", "plant"]:
        # Create AI-designed bush
        prop_objects = create_ai_bush()
        
    elif "raven_perch" == "well":
    ...
    if len(prop_objects) > 1:
        ...
        if prop_objects:
            ...
            final_prop.name = f"AI_raven_perch_19"
            
            print(f"✅ Created AI raven_perch with {len(prop_objects)} components")
...
def create_ai_tree():
    ...
    if trunk_style == "twisted":
        ...
        for i in range(max(2, DETAIL_COUNT // 2)):
            ...
            bpy.ops.mesh.primitive_cylinder_add(
                ...
                location=(23.0, 10.0, i * segment_height + segment_height/2)
            )
            segment = bpy.context.active_object
            segment.name = f"AI_twisted_segment_{i}_19"
        ...
        bpy.ops.mesh.primitive_cylinder_add(
            ...
            location=(23.0, 10.0, trunk_height * 0.4)
        )
        trunk = bpy.context.active_object
        trunk.name = f"AI_gnarled_trunk_19"
        ...
        bpy.ops.mesh.primitive_cylinder_add(
            ...
            location=(23.0, 10.0, trunk_height * 0.3)
        )
        trunk = bpy.context.active_object
        trunk.name = f"AI_split_main_19"
        ...
        for i in range(2):
            ...
            bpy.ops.mesh.primitive_cylinder_add(
                ...
                location=(23.0 + offset_x, 10.0 + offset_y, trunk_height * 0.75)
            )
            branch = bpy.context.active_object
            branch.name = f"AI_split_branch_{i}_19"
        ...
        bpy.ops.mesh.primitive_cylinder_add(
            ...
            radius=trunk_radius,
            depth=trunk_height,
            location=(23.0, 10.0, trunk_height/2)
        )
        trunk = bpy.context.active_object
        trunk.name = f"AI_straight_trunk_19"
        tree_parts.append(trunk)
    ...
    if canopy_shape == "round":
        bpy.ops.mesh.primitive_ico_sphere_add(
            subdivisions=2,
            radius=canopy_size,
            location=(23.0, 10.0, canopy_height)
        )
        canopy = bpy.context.active_object
        canopy.name = f"AI_round_canopy_19"
        ...
        canopy.name = f"AI_oval_canopy_19"
        tree_parts.append(canopy)
        ...
        for i in range(DETAIL_COUNT):
            ...
            bpy.ops.mesh.primitive_ico_sphere_add(
                subdivisions=1,
                radius=size,
                location=(23.0 + offset_x, 10.0 + offset_y, canopy_height + offset_z)
            )
            sphere = bpy.context.active_object
            sphere.name = f"AI_irregular_canopy_{i}_19"
        ...
        for i in range(cluster_count):
            ...
            cluster.name = f"AI_sparse_cluster_{i}_19"
        ...
        for i in range(frond_count):
            ...
            frond.name = f"AI_palm_frond_{i}_19"
        ...
        canopy = bpy.context.active_object
        canopy.name = f"AI_default_canopy_19"
        tree_parts.append(canopy)
...
def create_ai_rock():
    ...
    if rock_shape == "angular":
        bpy.ops.mesh.primitive_cube_add(
            size=base_size,
            location=(23.0, 10.0, rock_height/2)
        )
        ...
        bpy.ops.mesh.primitive_cone_add(
            ...
            location=(23.0, 10.0, rock_height * 0.75)
        )
        rock = bpy.context.active_object
        ...
        for i in range(DETAIL_COUNT // 2):
            ...
            bpy.ops.mesh.primitive_cone_add(
                ...
                location=(23.0 + offset_x, 10.0 + offset_y, rock_height * 0.35)
            )
            crystal = bpy.context.active_object
            crystal.name = f"AI_crystal_{i}_19"
    ...
    rock.name = f"AI_geometric_rock_19"
    ...
    if bush_shape == "spreading":
        ...
        for i in range(DETAIL_COUNT):
            ...
            bpy.ops.mesh.primitive_ico_sphere_add(
                subdivisions=1,
                radius=sphere_size,
                location=(23.0 + offset_x, 10.0 + offset_y, bush_height * 0.4)
            )
            sphere = bpy.context.active_object
            sphere.name = f"AI_spreading_bush_{i}_19"
        ...
        bpy.ops.mesh.primitive_ico_sphere_add(
            subdivisions=2,
            radius=bush_size,
            location=(23.0, 10.0, bush_height/2)
        )
        bush = bpy.context.active_object
        bush.scale = (1.0, 1.0, bush_height)
        bpy.ops.object.transform_apply(scale=True)
        bush.name = f"AI_geometric_bush_19"
...
def create_ai_well():
    ...
    bpy.ops.mesh.primitive_cylinder_add(
        ...
        depth=0.5,
        location=(23.0, 10.0, 0.25)
    )
    base = bpy.context.active_object
    base.name = f"AI_well_base_19"
    ...
    bpy.ops.mesh.primitive_cylinder_add(
        ...
        location=(23.0, 10.0, 0.5 * HEIGHT_MULT)
    )
    wall = bpy.context.active_object
    wall.name = f"AI_well_wall_19"
    ...
    if random.random() > 0.5:  # 50% chance of roof
        bpy.ops.mesh.primitive_cone_add(
            ...
            radius2=0.1,
            depth=0.8,
            location=(23.0, 10.0, HEIGHT_MULT + 0.4)
        )
        roof = bpy.context.active_object
        roof.name = f"AI_well_roof_19"
...
def create_ai_generic_prop():
    ...
    prop.name = f"AI_generic_prop_19"
...
try:
    created_objects = create_ai_unique_raven_perch()
    print(f"✅ Successfully created AI raven_perch with {len(created_objects)} components")
    ...
    print(f"❌ Error creating AI raven_perch: {e}")
    import traceback
    traceback.print_exc()
