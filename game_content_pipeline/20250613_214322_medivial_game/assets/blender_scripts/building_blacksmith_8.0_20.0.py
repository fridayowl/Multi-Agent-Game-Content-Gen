...
# Texture paths
textures = {"walls": "textures/stone_medieval_1e8e6849.png", "metal": "textures/metal_medieval_4bee8049.png"}

# Create blacksmith
bpy.ops.mesh.primitive_cube_add(
    size=1,
    location=(8.0, 20.0, 0.0)
)

building = bpy.context.active_object
building.name = "blacksmith_8.0_20.0"
building.scale = (8, 8, 6)

# Apply transforms
bpy.ops.object.transform_apply(location=False, rotation=False, scale=True)

# Add detailed geometry based on type
if "blacksmith" == "church":
    # Add tower for church
    bpy.ops.mesh.primitive_cube_add(
        size=1,
        location=(8.0, 24.0, 3.0)
    )
...
elif "blacksmith" == "blacksmith":
    # Add chimney for blacksmith
    bpy.ops.mesh.primitive_cylinder_add(
        radius=0.5,
        depth=3,
        location=(10.666666666666666, 20.0, 7.5)
    )
...
if "walls" in textures:
    wall_material = create_material_with_texture("medieval_blacksmith_walls", textures["walls"], base_color)
    building.data.materials.append(wall_material)
elif "roof" in textures:
    roof_material = create_material_with_texture("medieval_blacksmith_roof", textures["roof"], base_color)
    building.data.materials.append(roof_material)
else:
    # Fallback material
    material = bpy.data.materials.new(name="medieval_blacksmith")
    material.use_nodes = True
