...
# Prop #60: cactus
# AI Description: A unique desert cactus with distinctive characteristics
...
# AI Texture paths
ai_textures = {"surface": "complete_game_content/complete_game_20250617_113928_Generate_a_desert_oasis_tradin/ai_creative_assets/ai_textures/generic_desert_f5ff1d0c.png"}

# AI-Generated Geometry Parameters
HEIGHT_MULT = 1.1012711302515437
WIDTH_MULT = 1.0411154800677285
COMPLEXITY = "simple"
DETAIL_COUNT = 6
ASYMMETRY = 0.3751556504034027

print(f"🎨 Creating AI-Designed cactus:")
print(f"   📐 Height: {HEIGHT_MULT:.2f}x, Width: {WIDTH_MULT:.2f}x")
print(f"   🔧 Complexity: {COMPLEXITY}")
print(f"   ✨ Details: {DETAIL_COUNT}")
print(f"   🎭 Asymmetry: {ASYMMETRY:.2f}")

def create_ai_unique_cactus():
    """Create cactus with AI-determined unique characteristics"""
    
    prop_objects = []
    
    if "cactus" in ["tree", "oak_tree", "dead_tree", "palm_tree"]:
        # Create AI-designed tree
        prop_objects = create_ai_tree()
        
    elif "cactus" in ["rock", "stone", "boulder"]:
        # Create AI-designed rock
        prop_objects = create_ai_rock()
        
    elif "cactus" in ["bush", "shrub", "plant"]:
    ...
    if len(prop_objects) > 1:
        ...
        if prop_objects:
            ...
            final_prop.name = f"AI_cactus_59"
            
            print(f"✅ Created AI cactus with {len(prop_objects)} components")
...
try:
    created_objects = create_ai_unique_cactus()
    print(f"✅ Successfully created AI cactus with {len(created_objects)} components")
    ...
    print(f"❌ Error creating AI cactus: {e}")
    import traceback
    traceback.print_exc()
