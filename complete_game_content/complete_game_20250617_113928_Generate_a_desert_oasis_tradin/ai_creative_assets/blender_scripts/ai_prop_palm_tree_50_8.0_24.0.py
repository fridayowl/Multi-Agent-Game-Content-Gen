...
# Style Parameters: {'trunk_style': 'curved', 'canopy_shape': 'palm_fronds', 'branch_density': 'dense', 'leaf_type': 'palm', 'seasonal_state': 'spring', 'bark_texture': 'mossy', 'height_variation': 'tall'}
...
# AI Texture paths
ai_textures = {"bark": "complete_game_content/complete_game_20250617_113928_Generate_a_desert_oasis_tradin/ai_creative_assets/ai_textures/wood_desert_163028a5.png", "leaves": "complete_game_content/complete_game_20250617_113928_Generate_a_desert_oasis_tradin/ai_creative_assets/ai_textures/foliage_desert_4300cbe9.png"}

# AI-Generated Geometry Parameters
HEIGHT_MULT = 1.176188442965302
WIDTH_MULT = 1.203461412596578
COMPLEXITY = "complex"
DETAIL_COUNT = 5
ASYMMETRY = 0.15620572966801896
...
def create_ai_unique_palm_tree():
    ...
    if len(prop_objects) > 1:
        ...
        if prop_objects:
            ...
            final_prop.name = f"AI_palm_tree_50"
...
def create_ai_tree():
    ...
    if trunk_style == "twisted":
        ...
        for i in range(max(2, DETAIL_COUNT // 2)):
            ...
            bpy.ops.mesh.primitive_cylinder_add(
                ...
                location=(8.0, 24.0, i * segment_height + segment_height/2)
            )
            segment = bpy.context.active_object
            segment.name = f"AI_twisted_segment_{i}_50"
        ...
        bpy.ops.mesh.primitive_cylinder_add(
            ...
            location=(8.0, 24.0, trunk_height * 0.4)
        )
        trunk = bpy.context.active_object
        trunk.name = f"AI_gnarled_trunk_50"
        ...
        bpy.ops.mesh.primitive_cylinder_add(
            ...
            location=(8.0, 24.0, trunk_height * 0.3)
        )
        trunk = bpy.context.active_object
        trunk.name = f"AI_split_main_50"
        ...
        for i in range(2):
            ...
            bpy.ops.mesh.primitive_cylinder_add(
                ...
                location=(8.0 + offset_x, 24.0 + offset_y, trunk_height * 0.75)
            )
            branch = bpy.context.active_object
            branch.name = f"AI_split_branch_{i}_50"
        ...
        bpy.ops.mesh.primitive_cylinder_add(
            ...
            radius=trunk_radius,
            depth=trunk_height,
            location=(8.0, 24.0, trunk_height/2)
        )
        trunk = bpy.context.active_object
        trunk.name = f"AI_straight_trunk_50"
        tree_parts.append(trunk)
    ...
    if canopy_shape == "round":
        bpy.ops.mesh.primitive_ico_sphere_add(
            subdivisions=2,
            radius=canopy_size,
            location=(8.0, 24.0, canopy_height)
        )
        canopy = bpy.context.active_object
        canopy.name = f"AI_round_canopy_50"
        ...
        canopy.name = f"AI_oval_canopy_50"
        tree_parts.append(canopy)
        ...
        for i in range(DETAIL_COUNT):
            ...
            bpy.ops.mesh.primitive_ico_sphere_add(
                subdivisions=1,
                radius=size,
                location=(8.0 + offset_x, 24.0 + offset_y, canopy_height + offset_z)
            )
            sphere = bpy.context.active_object
            sphere.name = f"AI_irregular_canopy_{i}_50"
        ...
        for i in range(cluster_count):
            ...
            cluster.name = f"AI_sparse_cluster_{i}_50"
        ...
        for i in range(frond_count):
            ...
            frond.name = f"AI_palm_frond_{i}_50"
        ...
        canopy = bpy.context.active_object
        canopy.name = f"AI_default_canopy_50"
        tree_parts.append(canopy)
...
def create_ai_rock():
    ...
    if rock_shape == "angular":
        bpy.ops.mesh.primitive_cube_add(
            size=base_size,
            location=(8.0, 24.0, rock_height/2)
        )
        ...
        bpy.ops.mesh.primitive_cone_add(
            ...
            location=(8.0, 24.0, rock_height * 0.75)
        )
        rock = bpy.context.active_object
        ...
        for i in range(DETAIL_COUNT // 2):
            ...
            bpy.ops.mesh.primitive_cone_add(
                ...
                location=(8.0 + offset_x, 24.0 + offset_y, rock_height * 0.35)
            )
            crystal = bpy.context.active_object
            crystal.name = f"AI_crystal_{i}_50"
    ...
    rock.name = f"AI_rounded_rock_50"
    ...
    if bush_shape == "spreading":
        ...
        for i in range(DETAIL_COUNT):
            ...
            bpy.ops.mesh.primitive_ico_sphere_add(
                subdivisions=1,
                radius=sphere_size,
                location=(8.0 + offset_x, 24.0 + offset_y, bush_height * 0.4)
            )
            sphere = bpy.context.active_object
            sphere.name = f"AI_spreading_bush_{i}_50"
...
def create_ai_well():
    ...
    bpy.ops.mesh.primitive_cylinder_add(
        ...
        depth=0.5,
        location=(8.0, 24.0, 0.25)
    )
    base = bpy.context.active_object
    base.name = f"AI_well_base_50"
    ...
    bpy.ops.mesh.primitive_cylinder_add(
        ...
        location=(8.0, 24.0, 0.5 * HEIGHT_MULT)
    )
    wall = bpy.context.active_object
    wall.name = f"AI_well_wall_50"
    ...
    if random.random() > 0.5:  # 50% chance of roof
        bpy.ops.mesh.primitive_cone_add(
            ...
            radius2=0.1,
            depth=0.8,
            location=(8.0, 24.0, HEIGHT_MULT + 0.4)
        )
        roof = bpy.context.active_object
        roof.name = f"AI_well_roof_50"
...
def create_ai_generic_prop():
    ...
    prop.name = f"AI_generic_prop_50"
...
try:
    ...
    print(f"🎨 Style: {'trunk_style': 'curved', 'canopy_shape': 'palm_fronds', 'branch_density': 'dense', 'leaf_type': 'palm', 'seasonal_state': 'spring', 'bark_texture': 'mossy', 'height_variation': 'tall'}")
