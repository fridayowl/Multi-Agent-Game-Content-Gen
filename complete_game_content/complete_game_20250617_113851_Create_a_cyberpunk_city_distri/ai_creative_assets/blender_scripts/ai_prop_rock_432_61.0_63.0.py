...
# Style Parameters: {'shape': 'crystalline', 'surface': 'rough', 'size_category': 'small', 'formation': 'pile', 'weathering': 'ancient', 'mineral_type': 'limestone'}
...
# AI Texture paths
ai_textures = {"surface": "complete_game_content/complete_game_20250617_113851_Create_a_cyberpunk_city_distri/ai_creative_assets/ai_textures/stone_modern_8423f2f8.png"}

# AI-Generated Geometry Parameters
HEIGHT_MULT = 0.5794144128774285
WIDTH_MULT = 0.8437054722154168
COMPLEXITY = "simple"
DETAIL_COUNT = 5
ASYMMETRY = 0.23442779425183763
...
def create_ai_tree():
    ...
    if trunk_style == "twisted":
        ...
        for i in range(max(2, DETAIL_COUNT // 2)):
            ...
            bpy.ops.mesh.primitive_cylinder_add(
                ...
                location=(61.0, 63.0, i * segment_height + segment_height/2)
            )
            segment = bpy.context.active_object
            segment.name = f"AI_twisted_segment_{i}_432"
        ...
        bpy.ops.mesh.primitive_cylinder_add(
            ...
            location=(61.0, 63.0, trunk_height * 0.4)
        )
        trunk = bpy.context.active_object
        trunk.name = f"AI_gnarled_trunk_432"
        
        # Add gnarls (bumps)
        for i in range(DETAIL_COUNT):
            bump_radius = trunk_radius * 0.3
            angle = i * (2 * math.pi / DETAIL_COUNT)
            bump_x = 61.0 + math.cos(angle) * trunk_radius * 0.8
        ...
        bpy.ops.mesh.primitive_cylinder_add(
            ...
            location=(61.0, 63.0, trunk_height * 0.3)
        )
        trunk = bpy.context.active_object
        trunk.name = f"AI_split_main_432"
        ...
        for i in range(2):
            ...
            bpy.ops.mesh.primitive_cylinder_add(
                ...
                location=(61.0 + offset_x, 63.0 + offset_y, trunk_height * 0.75)
            )
            branch = bpy.context.active_object
            branch.name = f"AI_split_branch_{i}_432"
        ...
        bpy.ops.mesh.primitive_cylinder_add(
            ...
            radius=trunk_radius,
            depth=trunk_height,
            location=(61.0, 63.0, trunk_height/2)
        )
        trunk = bpy.context.active_object
        trunk.name = f"AI_straight_trunk_432"
        tree_parts.append(trunk)
    ...
    if canopy_shape == "round":
        bpy.ops.mesh.primitive_ico_sphere_add(
            subdivisions=2,
            radius=canopy_size,
            location=(61.0, 63.0, canopy_height)
        )
        canopy = bpy.context.active_object
        canopy.name = f"AI_round_canopy_432"
        ...
        canopy.name = f"AI_oval_canopy_432"
        tree_parts.append(canopy)
        ...
        for i in range(DETAIL_COUNT):
            ...
            bpy.ops.mesh.primitive_ico_sphere_add(
                subdivisions=1,
                radius=size,
                location=(61.0 + offset_x, 63.0 + offset_y, canopy_height + offset_z)
            )
            sphere = bpy.context.active_object
            sphere.name = f"AI_irregular_canopy_{i}_432"
        ...
        for i in range(cluster_count):
            ...
            cluster_x = 61.0 + math.cos(angle) * radius
            ...
            cluster.name = f"AI_sparse_cluster_{i}_432"
        ...
        for i in range(frond_count):
            ...
            frond_x = 61.0 + math.cos(angle) * frond_length * 0.7
            ...
            frond.name = f"AI_palm_frond_{i}_432"
        ...
        canopy = bpy.context.active_object
        canopy.name = f"AI_default_canopy_432"
        tree_parts.append(canopy)
...
def create_ai_rock():
    ...
    if rock_shape == "angular":
        bpy.ops.mesh.primitive_cube_add(
            size=base_size,
            location=(61.0, 63.0, rock_height/2)
        )
        ...
        bpy.ops.mesh.primitive_cone_add(
            ...
            location=(61.0, 63.0, rock_height * 0.75)
        )
        rock = bpy.context.active_object
        ...
        for i in range(DETAIL_COUNT // 2):
            ...
            bpy.ops.mesh.primitive_cone_add(
                ...
                location=(61.0 + offset_x, 63.0 + offset_y, rock_height * 0.35)
            )
            crystal = bpy.context.active_object
            crystal.name = f"AI_crystal_{i}_432"
    ...
    rock.name = f"AI_crystalline_rock_432"
    ...
    if bush_shape == "spreading":
        ...
        for i in range(DETAIL_COUNT):
            ...
            bpy.ops.mesh.primitive_ico_sphere_add(
                subdivisions=1,
                radius=sphere_size,
                location=(61.0 + offset_x, 63.0 + offset_y, bush_height * 0.4)
            )
            sphere = bpy.context.active_object
            sphere.name = f"AI_spreading_bush_{i}_432"
        ...
        bpy.ops.mesh.primitive_ico_sphere_add(
            subdivisions=2,
            radius=bush_size,
            location=(61.0, 63.0, bush_height/2)
        )
        bush = bpy.context.active_object
        bush.scale = (1.0, 1.0, bush_height)
        bpy.ops.object.transform_apply(scale=True)
        bush.name = f"AI_crystalline_bush_432"
...
def create_ai_well():
    ...
    bpy.ops.mesh.primitive_cylinder_add(
        ...
        depth=0.5,
        location=(61.0, 63.0, 0.25)
    )
    base = bpy.context.active_object
    base.name = f"AI_well_base_432"
    ...
    bpy.ops.mesh.primitive_cylinder_add(
        ...
        location=(61.0, 63.0, 0.5 * HEIGHT_MULT)
    )
    wall = bpy.context.active_object
    wall.name = f"AI_well_wall_432"
    ...
    if random.random() > 0.5:  # 50% chance of roof
        bpy.ops.mesh.primitive_cone_add(
            ...
            radius2=0.1,
            depth=0.8,
            location=(61.0, 63.0, HEIGHT_MULT + 0.4)
        )
        roof = bpy.context.active_object
        roof.name = f"AI_well_roof_432"
...
def create_ai_generic_prop():
    ...
    prop.name = f"AI_generic_prop_432"
...
try:
    ...
    print(f"🎨 Style: {'shape': 'crystalline', 'surface': 'rough', 'size_category': 'small', 'formation': 'pile', 'weathering': 'ancient', 'mineral_type': 'limestone'}")
