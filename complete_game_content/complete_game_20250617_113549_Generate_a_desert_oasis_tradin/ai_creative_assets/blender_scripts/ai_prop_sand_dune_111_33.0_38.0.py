...
# Prop #112: sand_dune
# AI Description: A unique desert sand_dune with distinctive characteristics
# Style Parameters: {'shape': 'geometric', 'size': 'small', 'texture': 'detailed', 'color_scheme': 'muted', 'complexity': 'moderate'}
...
# AI Texture paths
ai_textures = {"surface": "complete_game_content/complete_game_20250617_113549_Generate_a_desert_oasis_tradin/ai_creative_assets/ai_textures/generic_desert_0d00bc9d.png"}

# AI-Generated Geometry Parameters
HEIGHT_MULT = 1.0757621118751044
WIDTH_MULT = 1.2703562791007126
COMPLEXITY = "simple"
DETAIL_COUNT = 5
ASYMMETRY = 0.14677207838732126

print(f"🎨 Creating AI-Designed sand_dune:")
print(f"   📐 Height: {HEIGHT_MULT:.2f}x, Width: {WIDTH_MULT:.2f}x")
print(f"   🔧 Complexity: {COMPLEXITY}")
print(f"   ✨ Details: {DETAIL_COUNT}")
print(f"   🎭 Asymmetry: {ASYMMETRY:.2f}")

def create_ai_unique_sand_dune():
    """Create sand_dune with AI-determined unique characteristics"""
    
    prop_objects = []
    
    if "sand_dune" in ["tree", "oak_tree", "dead_tree", "palm_tree"]:
        # Create AI-designed tree
        prop_objects = create_ai_tree()
        
    elif "sand_dune" in ["rock", "stone", "boulder"]:
        # Create AI-designed rock
        prop_objects = create_ai_rock()
        
    elif "sand_dune" in ["bush", "shrub", "plant"]:
    ...
    if len(prop_objects) > 1:
        ...
        if prop_objects:
            ...
            final_prop.name = f"AI_sand_dune_111"
            
            print(f"✅ Created AI sand_dune with {len(prop_objects)} components")
...
try:
    created_objects = create_ai_unique_sand_dune()
    print(f"✅ Successfully created AI sand_dune with {len(created_objects)} components")
    print(f"🎨 Style: {'shape': 'geometric', 'size': 'small', 'texture': 'detailed', 'color_scheme': 'muted', 'complexity': 'moderate'}")
    print(f"🎯 Variations: 2")
except Exception as e:
    print(f"❌ Error creating AI sand_dune: {e}")
    import traceback
    traceback.print_exc()
