...
# Prop #28: skeleton
# AI Description: A unique spooky skeleton with distinctive characteristics
# Style Parameters: {'shape': 'geometric', 'size': 'medium', 'texture': 'smooth', 'color_scheme': 'natural', 'complexity': 'intricate'}
...
# AI Texture paths
ai_textures = {"surface": "complete_game_content/complete_game_20250616_195016_create_a_spooky_game/ai_creative_assets/ai_textures/generic_spooky_8982e7d6.png"}

# AI-Generated Geometry Parameters
HEIGHT_MULT = 1.0599933304692009
WIDTH_MULT = 0.9056612524031684
COMPLEXITY = "simple"
DETAIL_COUNT = 6
ASYMMETRY = 0.2320040236861853

print(f"🎨 Creating AI-Designed skeleton:")
print(f"   📐 Height: {HEIGHT_MULT:.2f}x, Width: {WIDTH_MULT:.2f}x")
print(f"   🔧 Complexity: {COMPLEXITY}")
print(f"   ✨ Details: {DETAIL_COUNT}")
print(f"   🎭 Asymmetry: {ASYMMETRY:.2f}")

def create_ai_unique_skeleton():
    """Create skeleton with AI-determined unique characteristics"""
    
    prop_objects = []
    
    if "skeleton" in ["tree", "oak_tree", "dead_tree", "palm_tree"]:
        # Create AI-designed tree
        prop_objects = create_ai_tree()
        
    elif "skeleton" in ["rock", "stone", "boulder"]:
        # Create AI-designed rock
        prop_objects = create_ai_rock()
        
    elif "skeleton" in ["bush", "shrub", "plant"]:
    ...
    if len(prop_objects) > 1:
        ...
        if prop_objects:
            ...
            final_prop.name = f"AI_skeleton_27"
            
            print(f"✅ Created AI skeleton with {len(prop_objects)} components")
...
def create_ai_tree():
    ...
    if trunk_style == "twisted":
        ...
        for i in range(max(2, DETAIL_COUNT // 2)):
            ...
            bpy.ops.mesh.primitive_cylinder_add(
                ...
                location=(14.0, 14.0, i * segment_height + segment_height/2)
            )
        ...
        bpy.ops.mesh.primitive_cylinder_add(
            ...
            location=(14.0, 14.0, trunk_height * 0.4)
        )
        ...
        bpy.ops.mesh.primitive_cylinder_add(
            ...
            location=(14.0, 14.0, trunk_height * 0.3)
        )
        trunk = bpy.context.active_object
        ...
        for i in range(2):
            ...
            bpy.ops.mesh.primitive_cylinder_add(
                ...
                location=(14.0 + offset_x, 14.0 + offset_y, trunk_height * 0.75)
            )
        ...
        bpy.ops.mesh.primitive_cylinder_add(
            ...
            radius=trunk_radius,
            depth=trunk_height,
            location=(14.0, 14.0, trunk_height/2)
        )
        trunk = bpy.context.active_object
    ...
    if canopy_shape == "round":
        bpy.ops.mesh.primitive_ico_sphere_add(
            subdivisions=2,
            radius=canopy_size,
            location=(14.0, 14.0, canopy_height)
        )
        ...
        for i in range(DETAIL_COUNT):
            ...
            bpy.ops.mesh.primitive_ico_sphere_add(
                subdivisions=1,
                radius=size,
                location=(14.0 + offset_x, 14.0 + offset_y, canopy_height + offset_z)
            )
...
def create_ai_rock():
    ...
    if rock_shape == "angular":
        bpy.ops.mesh.primitive_cube_add(
            size=base_size,
            location=(14.0, 14.0, rock_height/2)
        )
        ...
        bpy.ops.mesh.primitive_cone_add(
            ...
            location=(14.0, 14.0, rock_height * 0.75)
        )
        rock = bpy.context.active_object
        ...
        for i in range(DETAIL_COUNT // 2):
            ...
            bpy.ops.mesh.primitive_cone_add(
                ...
                location=(14.0 + offset_x, 14.0 + offset_y, rock_height * 0.35)
            )
    ...
    if bush_shape == "spreading":
        ...
        for i in range(DETAIL_COUNT):
            ...
            bpy.ops.mesh.primitive_ico_sphere_add(
                subdivisions=1,
                radius=sphere_size,
                location=(14.0 + offset_x, 14.0 + offset_y, bush_height * 0.4)
            )
            sphere = bpy.context.active_object
        ...
        bpy.ops.mesh.primitive_ico_sphere_add(
            subdivisions=2,
            radius=bush_size,
            location=(14.0, 14.0, bush_height/2)
        )
...
def create_ai_well():
    ...
    bpy.ops.mesh.primitive_cylinder_add(
        ...
        location=(14.0, 14.0, 0.5 * HEIGHT_MULT)
    )
    ...
    if random.random() > 0.5:  # 50% chance of roof
        bpy.ops.mesh.primitive_cone_add(
            ...
            radius2=0.1,
            depth=0.8,
            location=(14.0, 14.0, HEIGHT_MULT + 0.4)
        )
...
try:
    created_objects = create_ai_unique_skeleton()
    print(f"✅ Successfully created AI skeleton with {len(created_objects)} components")
    print(f"🎨 Style: {'shape': 'geometric', 'size': 'medium', 'texture': 'smooth', 'color_scheme': 'natural', 'complexity': 'intricate'}")
    print(f"🎯 Variations: 2")
except Exception as e:
    print(f"❌ Error creating AI skeleton: {e}")
    import traceback
    traceback.print_exc()
