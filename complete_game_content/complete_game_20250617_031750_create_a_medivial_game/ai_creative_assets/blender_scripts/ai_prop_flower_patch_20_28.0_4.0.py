...
# AI Texture paths
ai_textures = {"surface": "complete_game_content/complete_game_20250617_031750_create_a_medivial_game/ai_creative_assets/ai_textures/generic_medieval_ea1c6f96.png"}

# AI-Generated Geometry Parameters
HEIGHT_MULT = 1.2809201427702677
WIDTH_MULT = 1.113008362676771
COMPLEXITY = "simple"
DETAIL_COUNT = 2
ASYMMETRY = 0.3773984265801965
...
def create_ai_unique_flower_patch():
    ...
    if len(prop_objects) > 1:
        ...
        if prop_objects:
            ...
            final_prop.name = f"AI_flower_patch_20"
...
def create_ai_tree():
    ...
    if trunk_style == "twisted":
        ...
        for i in range(max(2, DETAIL_COUNT // 2)):
            ...
            bpy.ops.mesh.primitive_cylinder_add(
                ...
                location=(28.0, 4.0, i * segment_height + segment_height/2)
            )
            segment = bpy.context.active_object
            segment.name = f"AI_twisted_segment_{i}_20"
        ...
        bpy.ops.mesh.primitive_cylinder_add(
            ...
            location=(28.0, 4.0, trunk_height * 0.4)
        )
        trunk = bpy.context.active_object
        trunk.name = f"AI_gnarled_trunk_20"
        ...
        bpy.ops.mesh.primitive_cylinder_add(
            ...
            location=(28.0, 4.0, trunk_height * 0.3)
        )
        trunk = bpy.context.active_object
        trunk.name = f"AI_split_main_20"
        ...
        for i in range(2):
            ...
            bpy.ops.mesh.primitive_cylinder_add(
                ...
                location=(28.0 + offset_x, 4.0 + offset_y, trunk_height * 0.75)
            )
            branch = bpy.context.active_object
            branch.name = f"AI_split_branch_{i}_20"
        ...
        bpy.ops.mesh.primitive_cylinder_add(
            ...
            radius=trunk_radius,
            depth=trunk_height,
            location=(28.0, 4.0, trunk_height/2)
        )
        trunk = bpy.context.active_object
        trunk.name = f"AI_straight_trunk_20"
        tree_parts.append(trunk)
    ...
    if canopy_shape == "round":
        bpy.ops.mesh.primitive_ico_sphere_add(
            subdivisions=2,
            radius=canopy_size,
            location=(28.0, 4.0, canopy_height)
        )
        canopy = bpy.context.active_object
        canopy.name = f"AI_round_canopy_20"
        ...
        canopy.name = f"AI_oval_canopy_20"
        tree_parts.append(canopy)
        ...
        for i in range(DETAIL_COUNT):
            ...
            bpy.ops.mesh.primitive_ico_sphere_add(
                subdivisions=1,
                radius=size,
                location=(28.0 + offset_x, 4.0 + offset_y, canopy_height + offset_z)
            )
            sphere = bpy.context.active_object
            sphere.name = f"AI_irregular_canopy_{i}_20"
        ...
        for i in range(cluster_count):
            ...
            cluster.name = f"AI_sparse_cluster_{i}_20"
        ...
        for i in range(frond_count):
            ...
            frond.name = f"AI_palm_frond_{i}_20"
        ...
        canopy = bpy.context.active_object
        canopy.name = f"AI_default_canopy_20"
        tree_parts.append(canopy)
...
def create_ai_rock():
    ...
    if rock_shape == "angular":
        bpy.ops.mesh.primitive_cube_add(
            size=base_size,
            location=(28.0, 4.0, rock_height/2)
        )
        ...
        bpy.ops.mesh.primitive_cone_add(
            ...
            location=(28.0, 4.0, rock_height * 0.75)
        )
        rock = bpy.context.active_object
        ...
        for i in range(DETAIL_COUNT // 2):
            ...
            bpy.ops.mesh.primitive_cone_add(
                ...
                location=(28.0 + offset_x, 4.0 + offset_y, rock_height * 0.35)
            )
            crystal = bpy.context.active_object
            crystal.name = f"AI_crystal_{i}_20"
    ...
    rock.name = f"AI_organic_rock_20"
    ...
    if bush_shape == "spreading":
        ...
        for i in range(DETAIL_COUNT):
            ...
            bpy.ops.mesh.primitive_ico_sphere_add(
                subdivisions=1,
                radius=sphere_size,
                location=(28.0 + offset_x, 4.0 + offset_y, bush_height * 0.4)
            )
            sphere = bpy.context.active_object
            sphere.name = f"AI_spreading_bush_{i}_20"
        ...
        bpy.ops.mesh.primitive_ico_sphere_add(
            subdivisions=2,
            radius=bush_size,
            location=(28.0, 4.0, bush_height/2)
        )
        bush = bpy.context.active_object
        bush.scale = (1.0, 1.0, bush_height)
        bpy.ops.object.transform_apply(scale=True)
        bush.name = f"AI_organic_bush_20"
...
def create_ai_well():
    ...
    bpy.ops.mesh.primitive_cylinder_add(
        ...
        depth=0.5,
        location=(28.0, 4.0, 0.25)
    )
    base = bpy.context.active_object
    base.name = f"AI_well_base_20"
    ...
    bpy.ops.mesh.primitive_cylinder_add(
        ...
        location=(28.0, 4.0, 0.5 * HEIGHT_MULT)
    )
    wall = bpy.context.active_object
    wall.name = f"AI_well_wall_20"
    ...
    if random.random() > 0.5:  # 50% chance of roof
        bpy.ops.mesh.primitive_cone_add(
            ...
            radius2=0.1,
            depth=0.8,
            location=(28.0, 4.0, HEIGHT_MULT + 0.4)
        )
        roof = bpy.context.active_object
        roof.name = f"AI_well_roof_20"
...
def create_ai_generic_prop():
    ...
    prop.name = f"AI_generic_prop_20"
